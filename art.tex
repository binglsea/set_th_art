...
本文讨论新版公理集合论附录中的误译.其中对序类的误译明显产生悖论.
\end{abstract}
...
{\bf 113 THEOREM}(\cite{jkelley1975} 第268页)\,
{\sl 
...
{\bf 115 DEFINITION}(\cite{jkelley1975} 第269页)\,
{\sl 
...
如果按不加区别的错误译法, 我们从下面两条译文看到的是悖论: 一个序数不是集合而同时又必定是一个集合.

{\bf 定理 113}(\cite{jkelley2010zh}第185页)
{\kaishu
$R$是一个序数，但不是一个集.
}

{\bf 定义 115}(\cite{jkelley2010zh}第185页)
{\kaishu
$x$ 是一个序数当且仅当$x\in R$.
}
...
第 182 页倒数第二行, 定理 97:
$g^{-1}$ --- 此因     $f^{-1}$ 是保序的  ---此因 $g^{-1}$ 是保序的

第 184 页第三行 类$E$是 $\in$关系：
