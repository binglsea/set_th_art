...
非负整数是一种序数,而一个序数是本身为集合的一个序类.
但讨论序类和序数时并未正式确定有集合存在.
...
所以, 译本这样的翻译不恰当,值得商榷: “一个公理的许多结论, 如果..." 没有谓语, 动词改变意思成了复数名词.
...
2) 漏译(\cite{jkelley2010zh} 第 181页):
...
\subsection{定理 143  证明的误译}
1) 原文(\cite{jkelley1975} 第 274页 第15行):
Intuitively select $g(h)$ to be a nest in $x$ containing properly each previously selected nest.

2) 误译(\cite{jkelley2010zh} 第 189页 第4行): 
直观上所选的 $g(h)$ 是$x$ 中的套,它真正被包含在每个前面所选的套中.

3) 更正:
直观上所选的 $g(h)$ 是$x$ 中的套,它真正包含每个前面所选的套.
...
183 / 7 & is $r$-related to & $r^-$ 关系于 & $r$ 关系于 \\
184 / 3 & The class $E$ is the $\in$-relation. &类$E$是 $E$关系. & 类$E$是 $\in$关系. \\
186 /13 & $f(u) = g(f\vert u)$ &  $f(u) = g(f(u))$ &  $f(u) = g(f\vert u)$  \\
